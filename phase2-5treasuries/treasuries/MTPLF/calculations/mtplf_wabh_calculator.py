...
def load_tranche_data():
    """Load Metaplanet tranche data"""
    tranches = []
    
    # Get current directory and build path
    current_dir = os.path.dirname(os.path.abspath(__file__))
    data_file = os.path.join(current_dir, '..', 'data', 'mtplf_tranches_raw.csv')
...
def main():
    print("\n" + "="*70)
    print("METAPLANET - WABH CALCULATION")
    print("="*70)
    ...
    try:
        ...
        print("\n✓ MTPLF WABH calculation complete")
        print("="*70 + "\n")
        
    except Exception as e:
        print(f"\n✗ Error: {e}")
        print(f"Please check that mtplf_tranches_raw.csv exists in the data folder")
        print("="*70 + "\n")
